...
{
    int nbFils = 0;
	//sem = sem_open("test.txt", O_CREAT|O_EXCL, 666, 0);
	//sem_post(sem);
    /* -------------------------------------------------------------------- */
	...
	{
		sprintf(synchroFilename, "%s_%d", "sem", i + 1);
		CHECK_IFNULL(synchro[i] = sem_open(synchroFilename,O_CREAT|O_EXCL, 666, 0), "pb sem synchro");
	}
	sem_post(synchro[0]);
    for (int i = 0; i < nbFils; i++)
    {
   ...
   for (int i = 0; i < nbFils; i++)
	{
		sprintf(synchroFilename, "%s_%d", "sem", i + 1);
		CHECK(sem_close(synchro[i]), "pb sem synchro");
		sem_unlink(synchroFilename);
	}
   //sem_close(sem);
//	sem_unlink("test.txt");
    setTerm(YELLOW);
...
/*
    ...
    initialWait(no);
    CHECK(sem_wait(synchro[no]), "sem_wait error");
    fprintf(stderr, "\n\tFils %d : \tlancement effectif du processus %d,"
            " fils de %d\n", no + 1, getpid(), getppid());
    ...
    resetTerm();
    //fprintf(stderr, "\tFils %d : \tfin du processus %d\n",no + 1, getpid());
CHECK(sem_post(synchro[no+1]), "sem_post error");
    exit(no);
